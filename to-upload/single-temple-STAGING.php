...
.single.single-temple.postid-9577 .content-section img { width: 100%; }
	...
	li.related-items { border-radius: 15px; padding: 15px; border: 1px solid #FB8B24; background-color: #FFDDBE; text-align: center; box-shadow: 0px 0px 8px #00000050; }
	...
	/* --- Gallery guard ---
	   Pipeline-generated content (mr/hi etc.) can be one blob with uncleared
	   alignleft/alignright floated images. Those floats were overlapping into the
	   gallery and collapsing it into a black box. Contain each language block's
	   floats, and force the gallery to start below any floats. Safe for all langs. */
	.mr_lang::after, .en_lang::after, .hi_lang::after,
	.gj_lang::after, .ta_lang::after, .te_lang::after,
	.ml_lang::after, .kn_lang::after, .bn_lang::after, .pa_lang::after { content: ""; display: block; clear: both; }
	.hero { clear: both; }

	/* --- Layout parity across languages ---
	   Structure per article: h1, h3, seven images floated inline through the
	   text, then the "Main Features" h3 + ul. English has enough text that the
	   floats are used up before the h3, so it lays out correctly. The Indic
	   translations run 15-20% shorter (temple 29090: gj 5,046 chars vs
	   en 6,180) and run out of text early, so two things break.

	   1. The Main Features heading and list wrapped into the narrow gutter
	      beside a still-active float. clear:both puts them full width, which is
	      exactly where English already puts them -- so English is unchanged. */
	.content-section h3,
	.content-section ul { clear: both; }

	/*  2. The final image then floats with an empty column beside it, because
	      there is no text left to fill it. Nothing in CSS can conjure text, so
	      the last image stops floating and centres instead. Note this also
	      centres English's last image rather than floating it -- that is the
	      cost of making all ten languages render the same. */
	.dis_lang img:last-of-type { float: none; display: block; margin: 20px auto; }
	...
	<div class="container">
		<div class="content-section">
			...
			<div class="hi_lang dis_lang" style="display:none;">
				<?php
				$content = get_field('hi_translation');
				echo apply_filters('the_content', $content);
				?>
			</div>
			...
			<div class="gj_lang dis_lang" style="display:none;">
				<?php
				$content = get_field('gujarati_content');
				echo apply_filters('the_content', $content);
				?>
			</div>
			...
			<div class="ta_lang dis_lang" style="display:none;">
				<?php
				$content = get_field('tamil_content');   // was 'ta_translation' (empty); real field is 'tamil_content'
				echo apply_filters('the_content', $content);
				?>
			</div>
			...
			<div class="te_lang dis_lang" style="display:none;">
				<?php
				$content = get_field('telugu_content');  // was 'te_translation' (empty); real field is 'telugu_content'
				echo apply_filters('the_content', $content);
				?>
			</div>
			...
			<div class="ml_lang dis_lang" style="display:none;">
				<?php
				$content = get_field('malayalam_content');
				echo apply_filters('the_content', $content);
				?>
			</div>
			...
			<div class="kn_lang dis_lang" style="display:none;">
				<?php
				$content = get_field('kannada_content');
				echo apply_filters('the_content', $content);
				?>
			</div>
			...
			<div class="bn_lang dis_lang" style="display:none;">
				<?php
				$content = get_field('bengali_content');
				echo apply_filters('the_content', $content);
				?>
			</div>
			...
			<div class="pa_lang dis_lang" style="display:none;">
				<?php
				$content = get_field('punjabi_content');
				echo apply_filters('the_content', $content);
				?>
			</div>

			<audio id="temple-audio-player" style="display:none;">
				<source id="temple-audio-source" src="" type="audio/mpeg">
			</audio>
...
<?php /* Removed CDN jquery.slim 3.5.1 reload: it overwrote WordPress' full jQuery 3.7.1 that
        lightSlider is bound to, causing "$ is not a function" and the gallery rendering as a
        black box. WP already provides jQuery; do NOT reload it here. */ ?>
