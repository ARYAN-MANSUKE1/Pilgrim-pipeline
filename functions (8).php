...
<!-- 							< ?php echo get_the_post_thumbnail(); ?> -->
								...
								<div class="gj_lang dis_lang" style="display: none;">
									<h5><?php the_field('gujarati_title'); ?></h5>
									<?php echo '<span class="view-details-btn"> अधिक माहिती </span>'; ?>
								</div>
...
/**
 * Chrome-string i18n (nav labels, footer menu, social labels, copyright).
 *
 * These are a small FIXED set of strings for the whole site (does not grow
 * with the number of temples) — unlike temple content, they used to live in
 * Elementor and got wiped by the site migration. Storing them in a single WP
 * option instead means: (1) the pipeline translates each language's chrome
 * ONCE and pushes it here, (2) the theme reads the cached dictionary and
 * swaps text on language change, (3) any future migration that resets
 * Elementor no longer touches this — it survives independently in wp_options.
 *
 * English is always the fallback (baked into the JS below), so a missing or
 * not-yet-translated language shows English instead of going blank.
 */
add_action( 'rest_api_init', 'pilgrim_register_ui_i18n_route' );
function pilgrim_register_ui_i18n_route() {
    register_rest_route( 'pilgrim/v1', '/ui-i18n', array(
        'methods'             => 'POST',
        'callback'            => 'pilgrim_save_ui_i18n',
...
function pilgrim_save_ui_i18n( WP_REST_Request $req ) {
    $lang    = sanitize_key( (string) $req->get_param( 'lang' ) );
    $strings = $req->get_param( 'strings' );

    if ( ! $lang || ! is_array( $strings ) ) {
        return new WP_Error( 'pilgrim_bad_request', 'Expected { "lang": "xx", "strings": {...} }', array( 'status' => 400 ) );
    }

    $clean = array();
    foreach ( $strings as $key => $val ) {
        $clean[ sanitize_key( (string) $key ) ] = sanitize_text_field( (string) $val );
    }

    $all = get_option( 'pilgrim_ui_i18n', array() );
    if ( ! is_array( $all ) ) {
        $all = array();
    }
    $all[ $lang ] = $clean;
    update_option( 'pilgrim_ui_i18n', $all, false );

    return array( 'ok' => true, 'lang' => $lang, 'keys_saved' => count( $clean ) );
}

/**
 * Language registry -- the single source of truth for "which languages exist
 * and what's turned on for each." One WP option (pilgrim_languages), keyed by
 * language code:
 *   { "bn": { "name": "Bengali", "fields": true, "chrome": true, "visible": true }, ... }
 *
 *   fields  -- create Content/Audio/Title ACF fields for this language on the
 *              temple post type (via PHP registration below, so field
 *              EXISTENCE survives a future DB/migration reset -- only the
 *              content values are DB-only, same as before).
 *   chrome  -- include this language when nav menu + footer/header chrome
 *              are synced (dashboard "Add Language" / "Sync Languages").
 *   visible -- show this language as a selectable option in the language
 *              dropdown on the live site.
 *
 * Seeded on first use with the 10 languages already live on the site, with
 * fields=false for all of them: their Content/Audio/Title fields already
 * exist (mr/en/hi under legacy *_translation names from the original ACF UI
 * group; gj/ta/te/ml/kn/bn/pa wherever they've been created by hand). Turning
 * on "fields" here would register a SECOND, colliding field group under this
 * function's generic naming convention. Only flip "fields" on for a language
 * whose fields do not exist anywhere yet -- normally a brand-new language
 * added via the dashboard.
 */
...
add_action( 'rest_api_init', 'pilgrim_register_languages_routes' );
function pilgrim_register_languages_routes() {
    $perm = function () { return current_user_can( 'manage_options' ); };
    register_rest_route( 'pilgrim/v1', '/languages', array(
        'methods' => 'GET', 'callback' => function () { return pilgrim_get_languages(); },
        'permission_callback' => $perm,
    ) );
    register_rest_route( 'pilgrim/v1', '/languages', array(
        'methods' => 'POST', 'callback' => 'pilgrim_save_language',
        'permission_callback' => $perm,
    ) );
}

function pilgrim_save_language( WP_REST_Request $req ) {
    $code   = sanitize_key( (string) $req->get_param( 'code' ) );
    $name   = sanitize_text_field( (string) $req->get_param( 'name' ) );
    $native = sanitize_text_field( (string) $req->get_param( 'native' ) );
    if ( ! $code || ! $name ) {
        return new WP_Error( 'pilgrim_bad_request', 'Expected code + name.', array( 'status' => 400 ) );
    }
    $langs = pilgrim_get_languages();
    $langs[ $code ] = array(
        'name'    => $name,
        'native'  => $native ?: $name,
        'fields'  => (bool) $req->get_param( 'fields' ),
        'chrome'  => (bool) $req->get_param( 'chrome' ),
        'visible' => (bool) $req->get_param( 'visible' ),
    );
    update_option( 'pilgrim_languages', $langs, false );
    return array( 'ok' => true, 'languages' => $langs );
}

/**
 * Register Content / Audio / Title ACF fields for every language with
 * fields=true, via PHP (not the ACF admin UI) -- so the field DEFINITIONS
 * live in this file and survive a DB reset. Field names follow the site's
 * existing convention: {full_english_name}_content / _audio / _title,
 * lowercased (matches gujarati_content, malayalam_audio, bengali_title, etc).
 *
 * Deliberately a SEPARATE field group (own key, own panel) rather than
 * merged into the original "Temple Fields" group: registering under the
 * original group's key would make this PHP registration take priority over
 * the database version, so any future manual edit to "Temple Fields" via the
 * ACF admin UI would stop showing up. Keeping this as its own group means
 * "Temple Fields" stays fully free for manual editing forever, and only
 * THIS panel ("Temple Fields (managed languages)") is tool-managed -- add or
 * change a managed language here via the dashboard, not the ACF UI.
 *
 * Safe to run on every request: acf_add_local_field_group() is idempotent.
 */
add_action( 'acf/init', 'pilgrim_register_language_acf_fields' );
function pilgrim_register_language_acf_fields() {
    ...
    foreach ( $langs as $code => $lang ) {
        if ( empty( $lang['fields'] ) ) {
            continue;
        }
        $slug = sanitize_title( $lang['name'] );        // e.g. "bengali"
        $slug = str_replace( '-', '_', $slug );
        $fields[] = array(
            'key' => "field_pilgrim_{$code}_content", 'name' => "{$slug}_content",
            'label' => "{$lang['name']} Content", 'type' => 'wysiwyg', 'tabs' => 'all', 'toolbar' => 'full', 'media_upload' => 1,
        );
        $fields[] = array(
            'key' => "field_pilgrim_{$code}_audio", 'name' => "{$slug}_audio",
            'label' => "{$lang['name']} Audio", 'type' => 'file', 'return_format' => 'array',
        );
        $fields[] = array(
            'key' => "field_pilgrim_{$code}_title", 'name' => "{$slug}_title",
            'label' => "{$lang['name']} Title", 'type' => 'text',
        );
    }
    if ( empty( $fields ) ) {
        return;
    ...
    acf_add_local_field_group( array(
        'key' => 'group_pilgrim_languages',
        'title' => 'Temple Fields (managed languages)',
        'fields' => $fields,
        'location' => array( array( array(
            'param' => 'post_type', 'operator' => '==', 'value' => 'temple',
        ) ) ),
        // REQUIRED for the pipeline to read/write these fields via REST at all --
        // without this, ACF's REST layer silently drops any unrecognized key in
        // the "acf" object: writes return 200 OK but never persist, and reads
        // never include the field. The admin UI works either way (doesn't use
        // REST), which is why the input boxes showed up but stayed empty.
        'show_in_rest' => 1,
    ) );
}
                    ...
                    while ( $temple_query->have_posts() ) : $temple_query->the_post(); ?>
                        <div class="temple-marquee-item">
                            <a href="<?php echo esc_url( get_permalink() ); ?>" target="_blank">
                                ...
                                <div class="gj_lang dis_lang" style="display:none;"><h4><?php the_field('gujarati_title'); ?></h4></div>
                            </a>
                        </div>
                    <?php endwhile;
                endfor;
